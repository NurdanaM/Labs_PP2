...
fruits_lifetime = 10000     # Lifetime of the fruit in millisecons (10 sec)
...
# Function to generate fruit with random weight
def generate_fruit():
    while True:
        fr_x = random.randrange(1, width // 10) * 10
        fr_y = random.randrange(1, height // 10) * 10
        weight = random.randint(2, 10) # Random weight from 5 to 20
        fruit_coor = [fr_x, fr_y, weight, pygame.time.get_ticks()]  # Store position, weight, spawn time
...
while not done:
    ...
    if head_square == fruit_coor[:2]:  # compare only x and y coordinates
        fruit_eaten = True
        score += fruit_coor[2]  # Add fruit's  weight to the score
    ...
    if pygame.time.get_ticks() - fruit_coor[3] > fruits_lifetime:
        fruit_eaten = True # Treat as if fruit was eaten (it disappears)
